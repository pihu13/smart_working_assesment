...
<div class="main-content">
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.edit.category.post') }}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}

                <input type="hidden" name="slug" value="{{$category->slug}}" />
                <input type="hidden" name="old_cat_image_full" value="{{$category->cat_image}}" />
                <input type="hidden" name="cat_image_full_db" id="cat_image_full_db" value="">
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label for="name">Category Name <span style="color:red;">*</span></label>
                            <input class="form-control" id="name" minlength="1" maxlength="255" value="{{$category->name}}" title="Category Name" name="name" type="text" placeholder="Enter Category Name" autocomplete="off">
                            @if ($errors->has('name'))
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="file" name="cat_image_full" id="cat_image_full" class="form-control cat_image_full">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            <img src="{{ asset('/storage/'.$img)}}" id="cat_image_full_cls" width="100" height="100">
                        </div>
                    </div>

                    <?php
                    if (@$category->parentCatData && count($category->parentCatData) > 0) {
                        ?>
                        <input class="form-control color_code" id="color_code" value="" name="color_code" type="hidden" title="BG Color" placeholder="Please Enter Color Code" autocomplete="off">
                        <?php
                    } else {
                        ?>
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label for="name">Category Color <span style="color:red;">*</span></label>
                                <input class="form-control color_code" id="color_code" value="{{ @$category->color_code }}" name="color_code" type="text" title="BG Color" placeholder="Please Enter Color Code" autocomplete="off">
                                @if ($errors->has('color_code'))
                                <div class="invalid-feedback" style="display:block;">{{ $errors->first('color_code') }}</div>
                                @endif
                            </div>
                        </div>
                        <?php
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        jQuery("#color_code").colorpicker();
        
        jQuery(".reset_form").on("click", function () {
            window.location.href = '<?php echo route('admin.category.list'); ?>';
        });
