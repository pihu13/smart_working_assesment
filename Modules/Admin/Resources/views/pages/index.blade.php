...
<div class="main-content">
    ...
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header">Page List
<!--                <a href="{{route('admin.add.page')}}">
                    <span class="badge badge-primary float-right">Add CMS Page</span>
                </a>-->
            </div>
        </div>
                        ...
                        <div class="box-content">
                            <table id="dataTable" class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr class="remove_row_<?php echo $page->id; ?>">
                                        <td style="display:none;"></td>
                                        <th scope="row" class="sr-no"> {{$i++}}</th>
                                        <td>{{$page->title }}</td>
                                        <td>{{ date_format(@$page->updated_at,"d M Y") }}</td>
                                        <th scope="col">
                                            @if($page->status == 1) 
                                            <a href="javascript:void(0);" class="actions status_update" data-status="0" data-slug="{{ $page->slug }}">
                                                <span class="badge badge-success">Active</span>
                                            </a> 
                                            @else 
                                            <a href="javascript:void(0);" class="actions status_update" data-status="1" data-slug="{{ $page->slug }}">
                                                <span class="badge badge-danger">Inactive</span>
                                            </a> 
                                            @endif 
                                        </th>
                                        <td class="action" style="float:left;">
                                            <a class="icon-btn preview" href="{{ route('admin.view.page',[$page->slug]) }}">		
                                                <button type="button" itle="View" class="icon-btn preview"><i class="fal fa-eye"></i></button>
                                            </a>
                                            <a class="icon-btn edit" href="{{ route('admin.edit.page',[$page->slug]) }}">
                                                <button type="button" itle="Edit" class="icon-btn edit"><i class="fal fa-edit"></i></button>
                                            </a>
<!--                                            <a href="javascript:void(0);" onclick="confirmDelete('<?php //echo $page->slug; ?>',<?php //echo $page->id; ?>);"  class="actions" data-slug="{{ $page->slug }}">
