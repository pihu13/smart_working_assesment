            ...
            <div class="col-md-6">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb m-0 p-0">
                        ...
                        <li class="breadcrumb-item"><a href="{{route('admin.product.return.orders')}}">Return Order Manager</a></li>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <b>
                                                <?php
                                                if (@$order->orderRefund && !empty($order->orderRefund)) {
                                                    echo '<span class="badge badge-success">' . strtoupper('Amount Refund') . '</span>';
                                                } else {
    ...
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header">
                ...
                <div class="col-md-9 col-form-label float-right mr-2">
                    <?php
                    if (@$order->orderRefund && !empty($order->orderRefund)) {
                        ?>
                        <span class="badge badge-success float-right">Order Amount Refund</span>
                        <?php
                    } else {
                        ?>

                        <form action="{{ route("admin.product.refund.amount") }}" method="post" id="refund_frm" class="refund_frm">
                            @csrf
                            <input type="hidden" name="order_id" id="order_id" value="{{ @$order->id }}" />
                            <input type="hidden" name="user_id" id="user_id" value="{{ @$order->user_id }}" />
                            <input type="hidden" name="user_email_address" id="user_email_address" value="{{ @$order->userDetails->email }}" />


                            <a href="javascript:void(0);" class="cls_refund">
                                <span class="badge badge-primary float-right mr-2">Pay Now For Refund Amount</span>
                            </a>
                            <div class="form-check form-check-inline form-check-sm mr-2 float-right">
                                <input class="form-check-input" id="inline-checkbox2" type="checkbox" name="delivery_charges_amount" value="{{ (@$order->delivery_charges_amount)?$order->delivery_charges_amount:"0" }}">
                                <label class="form-check-label" for="inline-checkbox2">
                                    <b>
                                        {{ (@$order->delivery_charges_amount)?$order->currency_symbol.$order->delivery_charges_amount:@$order->currency_symbol."0" }} Delivery Charge
                                    </b>
                                </label>
                            </div>
                            <div class="form-check form-check-inline form-check-sm mr-2 float-right">
                                <input class="form-check-input" id="inline-checkbox3" type="checkbox" name="product_tax" value="{{ (@$order->product_tax)?$order->product_tax:"0" }}">
                                <label class="form-check-label" for="inline-checkbox3">
                                    <b>
                                        {{ (@$order->product_tax)?$order->currency_symbol.$order->product_tax:@$order->currency_symbol."0" }} Tax Amount
                                    </b>
                                </label>
                            </div>
                            <div class="form-check form-check-inline form-check-sm mr-2 float-right">
                                <input class="form-check-input" id="inline-checkbox4" type="checkbox" name="tip_amount" value="{{ (@$order->tip_amount)?$order->tip_amount:"0" }}">
                                <label class="form-check-label" for="inline-checkbox4">
                                    <b>
                                        {{ (@$order->tip_amount)?$order->currency_symbol.$order->tip_amount:@$order->currency_symbol."0" }} Tip Amount
                                    </b>
                                </label>
                            </div>
                            <div class="form-check form-check-inline form-check-sm mr-2 float-right">
                                <input class="form-check-input" id="inline-checkbox1" type="checkbox" checked="checked" name="sub_total" value="{{ @$order->sub_total }}">
                                <label class="form-check-label" for="inline-checkbox1">
                                    <b>
                                        {{ @$order->currency_symbol.$order->sub_total }} Subtotal
                                    </b>
                                </label>
                            </div>
                        </form>
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        jQuery(".cls_refund").on("click", function () {
            jQuery("#refund_frm").submit();
        });
    });
</script>
@endsection
